...
class ReplayBuffer(object):
    ...
    def sample_planning(self, batch_size, horizon):
        end_idxs = np.random.randint(
            horizon, self.capacity if self.full else self.idx, size=batch_size
        )

        obses = self.obses[end_idxs - horizon]
        next_obses = self.next_obses[end_idxs]
        actions, rewards, not_dones, not_dones_no_max = [], [], [], []
        for t in range(horizon):
            actions.append(self.actions[end_idxs - horizon + t])
            rewards.append(self.rewards[end_idxs - horizon + t])
            not_dones.append(self.not_dones[end_idxs - horizon + t])
            not_dones_no_max.append(self.not_dones_no_max[end_idxs - horizon + t])

        actions = np.stack(actions)
        rewards = np.stack(rewards).squeeze(2)
        not_dones = np.stack(not_dones)
        not_dones_no_max = np.stack(not_dones_no_max)
