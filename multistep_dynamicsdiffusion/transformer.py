...
class TransformerForDiffusion(ModuleAttrMixin):
    def __init__(
        ...
        T_cond = (
            1 + 1 + horizon
        )  # diffusion_time(1) + init_state(1) + seq of actions (horizon)
        ...
        self.cond_obs_emb = nn.Linear(input_dim, n_emb)  # input_dim = cond_dim

        self.cond_act_emb = nn.Linear(act_dim, n_emb)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.encoder_only:
            ...
            cond_obs_emb = self.cond_obs_emb(initial_state)
            # (B,1,n_emb)
            cond_act_emb = self.cond_act_emb(actions)
            # (B,T,n_emb)
            cond_embeddings = torch.cat(
                [cond_embeddings, cond_obs_emb, cond_act_emb], dim=1
            )
