...
from dynamicsdiffusion import utils
...
class EvalVis:
    def __init__(self, args):
        ...
        if os.path.exists(fname):
            self.exp = torch.load(fname, map_location=args.device)
    ...
    def run_episode(self, seed, create_vid):
        ...
        while not done:
            ...
            if self.exp.cfg.normalize_obs:
                ...
                obs = (obs - mu) / sigma
            obs = torch.FloatTensor(obs).to(args.device)

            if args.mode == "mean":
                action_seq, _, _ = self.exp.agent.dx.unroll_policy(
                    obs.unsqueeze(0), exp.agent.actor, sample=False, last_u=True
                )
            elif args.mode == "sample":
                action_seq, _, _ = self.exp.agent.dx.unroll_policy(
                    obs.unsqueeze(0), exp.agent.actor, sample=True, last_u=True
                )
            elif args.mode == "ctrl":
            ...
            action = action_seq[0]
            action = action.clamp(
                min=env.action_space.low.min(), max=env.action_space.high.max()
            )
            ...
            if create_vid:
                ...
                def f():
                    preds_fname = os.path.join(episode_dir, f"preds_{step:04d}.png")
                    self.plot_obs_rew(
                    ...
                    if self.args.vid_mode == "highlight":
                        ...
                        os.system(
                            f"convert +append {env_fname} {fname} -resize x300 {fname}"
                        )
                        ...
                        os.system(
                            f"convert -gravity center {env_fname} -resize x700 {env_fname}"
                        )
                        os.system(
                            "convert -gravity center +append "
                        ...
                        os.system(
                            "convert -gravity center +append -resize x700 "
                            f"{env_fname} {preds_fname} {fname}"
                        )
                        os.system(
                            "convert -gravity center -append -resize 1200x "
    ...
    def plot_obs_rew(self, true_xs, pred_xs, true_rews, pred_rews, pred_dones, fname):
        ...
        for i in range(state_dim):
            ...
            for j in range(len(pred_xs)):
                (p,) = ax.plot(utils.to_np(pred_xs[j][:, i]), alpha=1.0, color=color)
                color = p.get_color()
        ...
        for j in range(len(pred_rews)):
            rew_ax.plot(utils.to_np(pred_rews[j]), alpha=1.0, color=color)
        ...
        if self.args.show_dones:
            ...
            for j in range(len(pred_dones)):
                done_ax.plot(utils.to_np(pred_dones[j]), alpha=1.0)
...
def add_label(ax, text, fontsize, xpos=0.0, ypos=1.0):
    ax.text(
        xpos,
        ypos,
        text,
